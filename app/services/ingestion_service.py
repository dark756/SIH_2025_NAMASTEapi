...
from app.services.ayush_translation import translator
from app.models.tm2_data import TM2RawRecord, TM2ProcessedRecord
from app.models.emr_models import EMRRecord, EMRPatient, EMRCondition, EMREncounter, EMRObservation, EMRStatistics
...
class TM2IngestionService:
    ...
    async def process_tm2_file(self, file_content: BinaryIO, filename: str) -> Dict[str, Any]:
        ...
        with HealthcareOperationContext("tm2_file_processing"):
            ...
            try:
                ...
                # Clean and show statistics before further processing
                cleaned_data, cleanup_stats = self._clean_and_summarize_data(raw_data)
                logger.info(
                ...
                # Convert cleaned data to EMR format
                emr_records, emr_stats = self._convert_to_emr(cleaned_data)
                logger.info(
                    "EMR conversion completed",
                    processing_id=processing_id,
                    **emr_stats
                )
                
                # Process records in batches (store and submit)
                processing_results = await self._process_records_batch(
                    cleaned_data, processing_id
                )
                
                # Update global statistics
                self._update_processing_stats(processing_results)
                
                # Convert EMR records to dictionaries for JSON serialization
                emr_output_dicts = []
                for emr_record in emr_records:
                    emr_output_dicts.append(emr_record.model_dump())

                # Prepare final result including EMR output and stats
                result = {
                    "processing_id": processing_id,
                    "filename": filename,
                    "status": "completed",
    ...
    def _clean_and_summarize_data(self, raw_data: List[Dict[str, Any]]) -> tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """
        Clean the raw data and generate summary statistics.
        ...
        for record in raw_data:
            ...
            # Validate record
            if not self._validate_record_for_processing(cleaned_record):
            ...
            for field in field_counts:
                if cleaned_record.get(field):
                    field_counts[field] += 1

            # Update severity distribution
            severity = cleaned_record.get("severity", "Unknown")
            stats["severity_distribution"][severity] = stats["severity_distribution"].get(severity, 0) + 1

            # Update system type distribution
            system_type = cleaned_record.get("system_type", "Unknown")
            stats["system_type_distribution"][system_type] = stats["system_type_distribution"].get(system_type, 0) + 1

            # Update date range
            diagnosis_date = cleaned_record.get("diagnosis_date")
            if diagnosis_date:
                try:
                    parsed_date = date_parser.parse(str(diagnosis_date))
        ...
        for field, count in field_counts.items():
            stats["field_completeness"][field] = (count / len(cleaned_data)) * 100 if cleaned_data else 0

        # Calculate data quality score
        completeness_avg = sum(stats["field_completeness"].values()) / len(stats["field_completeness"])
        duplicate_rate = (stats["duplicates_removed"] / stats["original_records"]) * 100 if stats["original_records"] > 0 else 0
        invalid_rate = (stats["invalid_records_removed"] / stats["original_records"]) * 100 if stats["original_records"] > 0 else 0

        stats["data_quality_score"] = max(0, completeness_avg - duplicate_rate - invalid_rate)

        return cleaned_data, stats

    def _clean_single_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        """
        Clean a single record by normalizing data and fixing common issues.
        ...
        for key, value in record.items():
            ...
            if isinstance(value, str):
                ...
                if key in ["system_type", "severity"]:
                    cleaned_value = cleaned_value.title()

                # Clean TM2 codes
                if key == "tm2_code":
                    cleaned_value = cleaned_value.upper().strip()
                    if not cleaned_value.startswith("TM2."):
                        cleaned_value = f"TM2.{cleaned_value}"

                # Clean IDs
                if key in ["patient_id", "practitioner_id"]:
                    cleaned_value = re.sub(r'[^a-zA-Z0-9_-]', '', cleaned_value.upper())
    ...
    def _validate_record_for_processing(self, record: Dict[str, Any]) -> bool:
        """
        Validate if a record is suitable for further processing.
        ...
        """
        required_fields = ["patient_id", "tm2_code", "condition_name", "diagnosis_date"]
        ...
        if not re.match(r'^TM2\.[A-Z0-9]+\.[A-Z0-9]+$', tm2_code):
            return False
        ...
        if diagnosis_date:
            try:
                date_parser.parse(str(diagnosis_date))
            except:
                return False

        return True

    def _convert_to_emr(self, cleaned_data: List[Dict[str, Any]]) -> tuple[List[EMRRecord], Dict[str, Any]]:
        """
        Convert cleaned TM2 data to EMR format.

        Args:
            cleaned_data: List of cleaned TM2 record dictionaries

        Returns:
            Tuple of (emr_records, statistics)
        """
        emr_records = []
        stats = {
        ...
        for record in cleaned_data:
            try:
                # Translate AYUSH/NAMASTE condition name to English if applicable
                original_condition_name = record.get("condition_name", "")
                translated_condition_name = translator.translate_condition(original_condition_name)

                # Also translate system type and severity if needed
                translated_system_type = translator.translate_system_type(record.get("system_type", ""))
                translated_severity = translator.translate_severity(record.get("severity", ""))
                ...
                if patient_id not in patients_map:
                    patient = EMRPatient(
                        ...
                        given_name=f"Patient {patient_counter}",
                        family_name="",
                        gender="Unknown",
                        birth_date=None,
                        address="",
                ...
                # Create condition with translated English name
                condition = EMRCondition(
                    condition_id=str(uuid4()),
                    patient_id=patient.patient_id,
                    condition_name=translated_condition_name,
                    icd_code=None,
                    tm2_code=record.get("tm2_code", ""),
                    system_type=record.get("system_type", ""),
                    severity=record.get("severity", ""),
                    diagnosis_date=self._parse_date(record.get("diagnosis_date", "")),
                ...
                encounter = EMREncounter(
                    ...
                    patient_id=patient.patient_id,
                    encounter_type="traditional_medicine_consultation",
                    encounter_date=self._parse_date(record.get("diagnosis_date", "")),
                    practitioner_id=record.get("practitioner_id", ""),
                    conditions=[condition.condition_id]
                )
                stats["encounters_created"] += 1

                # Create observation with translated English condition name
                observation = EMRObservation(
                    observation_id=str(uuid4()),
                    patient_id=patient.patient_id,
                    encounter_id=encounter.encounter_id,
                    concept="TM2_CONDITION",
                    value=translated_condition_name,
                    units=None,
                    observation_date=self._parse_date(record.get("diagnosis_date", "")),
                    practitioner_id=record.get("practitioner_id", "")
                )
                ...
                emr_record = EMRRecord(
                    ...
                    metadata={
                        ...
                        "system_type": record.get("system_type"),
                        "severity": record.get("severity"),
                        "original_condition_name": original_condition_name
                    }
                )
                ...
                logger.warning(
                    f"Failed to convert record to EMR format: {str(e)}",
                    extra={"patient_id": record.get("patient_id")}
                )
